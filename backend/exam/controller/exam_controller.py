...
router = APIRouter(tags=["Exams"], dependencies=[require_role(Role.FREEMIUM)])
...
async def list_exam_history(
    ...
    items = await exam_service.list_history(user_id=user.id)
    return [
...
async def get_exam_session(
    ...
    try:
        details = await exam_service.get_session_details(user_id=user.id, session_id=session_id)
    except ValueError as exc:
...
async def submit_exam_session(
    ...
    try:
        result = await exam_service.submit_session(user_id=user.id, session_id=session_id)
    except ValueError as exc:
        raise _error_for_service_exception(exc)
...
async def get_exam_result(
    ...
    try:
        result = await exam_service.get_result(user_id=user.id, session_id=session_id)
    except ValueError as exc:
        raise _error_for_service_exception(exc)
