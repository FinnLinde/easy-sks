...
class ExamService:
    ...
    async def list_history(self) -> list[ExamSessionHistoryItem]:
        sessions = await self._exam_repo.list_completed_sessions()
        return [
            ExamSessionHistoryItem(
                session_id=session.id,
    ...
    async def get_session_details(self, session_id: str) -> ExamSessionDetails:
        session = await self._exam_repo.get_session(session_id=session_id)
        if session is None:
    ...
    async def submit_session(self, session_id: str) -> ExamSessionResult:
        session = await self._exam_repo.get_session(session_id=session_id)
        if session is None:
            raise ValueError(f"Exam session {session_id!r} not found")
    ...
    async def get_result(self, session_id: str) -> ExamSessionResult:
