...
router = APIRouter(tags=["Navigation"], dependencies=[require_role(Role.FREEMIUM)])
...
async def list_navigation_history(
    ...
    items = await service.list_history(user_id=user.id)
    return [
...
async def get_navigation_session(
    ...
    try:
        details = await service.get_session_details(user_id=user.id, session_id=session_id)
    except ValueError as exc:
...
async def submit_navigation_session(
    ...
    try:
        result = await service.submit_session(user_id=user.id, session_id=session_id)
    except ValueError as exc:
        raise _error_for(exc)
...
async def get_navigation_result(
    ...
    try:
        result = await service.get_result(user_id=user.id, session_id=session_id)
    except ValueError as exc:
        raise _error_for(exc)
    return _result_to_out(result)
