...
class NavigationService:
    ...
    async def get_session_details(
        ...
        session = await self._repo.get_session(user_id=user_id, session_id=session_id)
        if session is None:
    ...
    async def list_history(self, user_id: str) -> list[NavigationSessionHistoryItem]:
        sessions = await self._repo.list_completed_sessions_for_user(user_id)
        return [
            NavigationSessionHistoryItem(
                session_id=s.id,
    ...
    async def save_answer(
        ...
        answer = await self._repo.get_answer(
