...
- KI-Antwortauswertung ist fuer die Exam-Session integriert (OpenAI + deterministischer Fallback ohne API-Key).
- Exam-Datenmodell, Session-Verwaltung, API-Endpunkte und UI sind umgesetzt.
...
| `score`             | Float            | 0.0 bis 2.0 (0.5er Schritte, nullable, nach Eval) |
...
1. **Start**: Nutzer waehlt einen festen Bogen (1–15). System erstellt `exam_session`, laedt die 30 zugehoerigen Karten (`WHERE sheet_number = ANY(exam_sheets)`), startet Timer.
...
3. **Abgabe / Timeout**: Timer wird bei `0` visuell als abgelaufen markiert, die Session bleibt aber editierbar bis zur expliziten Abgabe. Bei Abgabe nach Ablauf wird `time_over=true` gesetzt und `passed=false` erzwungen.
...
Antworte als JSON: { "score": 0..2 (0.5er Schritte), "is_correct": bool, "errors": [...], "feedback": "..." }
```
...
| GET     | `/exam-sessions`                  | Historie abgeschlossener Sessions    |
...
- Optionale ENV-Variablen fuer OpenAI-Evaluator: `EXAM_AI_OPENAI_API_KEY` (alternativ `OPENAI_API_KEY`), `EXAM_AI_OPENAI_MODEL`, `EXAM_AI_OPENAI_TIMEOUT_SECONDS`.

## Akzeptanzkriterien

- [x] Nutzer kann einen der 15 Pruefungsboegen auswaehlen und eine Session starten.
- [x] Timer laeuft sichtbar herunter; bei Ablauf wird die Session markiert und spaeter als nicht bestanden gewertet.
- [x] Nutzer kann Freitext-Antworten eingeben und zwischen Fragen navigieren.
- [x] Nach Abgabe werden alle Antworten per KI bewertet (OpenAI, mit Fallback ohne Key).
- [x] Ergebnis zeigt Gesamtpunktzahl, Bestanden/Nicht-bestanden und pro Frage: Score, Feedback, Fehler.
- [x] Sessiondaten sind nutzerbezogen gespeichert und wiederabrufbar.
- [x] Nutzer sieht eine Historie abgeschlossener Pruefungssessions.
...
- [x] Datenmodell finalisieren und Migration erstellen.
- [x] Exam-Service mit Session-Lifecycle implementieren.
- [x] API-Endpunkte bauen.
- [x] KI-Bewertungs-Integration umsetzen.
- [x] Frontend: Pruefungsauswahl und Simulations-UI.
- [x] Frontend: Ergebnis-Screen.
- [x] Tests ergaenzen.

## Offene Fragen

- Keine offenen Produktfragen fuer den MVP. Festgelegt: `39/60` Punkte zum Bestehen, kein Session-Resume, Historie enthalten.
