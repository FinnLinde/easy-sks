...
1. `ESKS-001` Multi-User-Fortschritt einführen (kritische Grundlage)
2. `ESKS-002` Frontend Login + Token-Injektion
3. `ESKS-003` User-Provisioning & Entitlement-Basis
4. `ESKS-004` Freemium-Regeln & Enforcement
5. `ESKS-005` Abo-Zahlung (Stripe) + Premium-Freischaltung
6. `ESKS-006` Dashboard MVP
7. `ESKS-007` Strukturierte Lernpfade/Kategorien
8. `ESKS-008` Pruefungssimulation MVP
9. `ESKS-009` KI-Antwortbewertung (Text) MVP
10. `ESKS-010` Spracheingabe + Transkription
11. `ESKS-011` Produktions-Infrastruktur-Basis
